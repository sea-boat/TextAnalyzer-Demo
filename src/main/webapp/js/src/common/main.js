var ip = "http://132.122.236.93:8080/";
var ends = "";
// var ip = "test/"
// var ends = ".json";
...
function inithalfMonthInfoChart(data)
{
    ...
    for(var i = 0; i < 15; i++)
    {
        // let date = Math.ceil(Math.random() * 10) + "-" +
...
{
    // dayInfoChartYAixs = ['07-12', '07-13', '07-14', '07-15', '07-16',
	// '07-17', '07-18'];
...
var word2vecBase =
{
    'content': function()
    {
        var content = $("#word2vecBaseContent").html();
        return content;
    },
...
var textClassifyBase =
{
    'content': function()
    {
        var content = $("#textClassifyBaseContent").html();
        return content;
    },
...
var addressExtractBase =
{
    'content': function()
    {
        var content = $("#addressExtractBaseContent").html();
        return content;
    },
...
var traingingBase =
{
    'content': function()
    {
        var content = $("#traingingBaseContent").html();
        return content;
    },
    ...
    'chart': function()
    {
        addTraingingBaseDiv(['traingingBaseSearch', 'keyword=']);
        // setTimeout('waterfall()', 400);
    }
}
...
layui.use(['form','table'], function(){
            ...
            form.on('submit(extract_btn)', function(data){
            	var myText = $("#myText").val()
            	$.ajax(
                            {
                                url: ip + "TextAnalyzer-Demo/hotword",
                                type: "POST",
                                data: "myText=" + myText,
                                dataType:"jsonp",
                                success: function (data)
                                {
                                	...
                                	table.render({
                                	    ...
                                	    ,cellMinWidth: 100 
                                	    ,cols: [[
                                	       {field:'keyword', width:100, title: '关键词'}
                                	      ,{field:'frequcence', width:100, title: '词频', sort: true}
                                	      ,{field:'score', width:200, title: '得分', sort: true}
                                	    ]]
                                	 ,data: values
                                	  });
                                }
...
layui.use(['form','table'], function(){
	var form = layui.form,layer=layui.layer,table = layui.table;
    form.on('submit(text_predict_btn)', function(data){
    	var myTestText = $("#myTestText").val()
    	$.ajax(
                    {
                        url: ip + "TextAnalyzer-Demo/predict",
                        type: "POST",
                        data: "myTestText=" + myTestText,
                        dataType:"jsonp",
                        success: function (data)
                        {
                        	var values =[];
                        	values[0]= {"class":data.predict,"text":myTestText};
                        	table.render({
                        	    elem: '#showPredictText'
                        	    ,cellMinWidth: 200 
                        	    ,cols: [[
                        	       {field:'class', width:200, title: '分类预测'},
                        	       {field:'text', width:800, title: '文本'}
                        	    ]]
                        	 ,data: values
                        	  });
                        }
...
layui.use(['form','table'], function(){
	var form = layui.form,layer=layui.layer,table = layui.table;
    form.on('submit(address_extract_btn)', function(data){
    	var myAddressText = $("#myAddressText").val()
    	$.ajax(
                    {
                        url: ip + "TextAnalyzer-Demo/address",
                        type: "POST",
                        data: "myAddressText=" + myAddressText,
                        dataType:"jsonp",
                        success: function (data)
                        {
                        	var strs= new Array(); 
                        	...
                        	table.render({
                        	    elem: '#showAddressText'
                        	    ,cellMinWidth: 200 
                        	    ,cols: [[
                        	       {field:'addr', width:200, title: '地址信息'}
                        	    ]]
                        	 ,data: values
                        	  });
                        }
...
layui.use(['form','table'], function(){
	var form = layui.form,layer=layui.layer,table = layui.table;
    form.on('submit(word_sim_btn)', function(data){
    	var kv = data.field;
    	$.ajax(
                    {
                        url: ip + "TextAnalyzer-Demo/similarity",
                        type: "POST",
                        data: "word1=" + kv.word1+"&word2="+kv.word2+"&type=word",
                        dataType:"jsonp",
                        success: function (data)
                        {
                        	$("#wordsim").html("相似性为："+data.similarity);
                        }
                    }
                )
        return false;
...
layui.use(['form','table'], function(){
	var form = layui.form,layer=layui.layer,table = layui.table;
    form.on('submit(sentence_sim_btn)', function(data){
    	var kv = data.field;
    	$.ajax(
                    {
                        url: ip + "TextAnalyzer-Demo/similarity",
                        type: "POST",
                        data: "sentence1=" + kv.sentence1+"&sentence2="+kv.sentence2+"&type=sentence",
                        dataType:"jsonp",
                        success: function (data)
                        {
                        	$("#sentencesim").html("相似性为："+data.similarity);
                        }
                    }
                )
        return false;
